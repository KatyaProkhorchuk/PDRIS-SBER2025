...
Запуск 
    `sh deploy.sh` - установка всех необходимых объектов
    `minikube tunnel` - создать сетевой мост между локальной машиной и кластером Kubernetes
    `sh senf-log.sh` - отправка логов 50 раз (для теста)
Так же для проверки `cronjob` был добавлен sleep на 5 минут, что бы убедиться в том, что все работает корректно